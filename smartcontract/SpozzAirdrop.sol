...
contract SpozzAirdrop is Ownable {
    ...
    uint256 airdropAmountPerAvatar = 50e9;
    uint256 airdropAmountPerNFT = 10e9;
    uint256 airdropAmountPerSpecialNFT = 30e9;
    uint256 airdropAmountForWhiteList = 30e9;
    uint256 airdropAmountForFirstComer = 10e9;
    ...
    function airdrop(address[] memory nfts, address[] memory xnfts, uint256[] memory ids, uint256[] memory xids) public  {
        ...
        if(isWhitelist[msg.sender] == true){
            spozzAmount.add(airdropAmountForWhiteList);
        }

        uint256 avatarCnt = getAvatarCnt(1);
        if(avatarCnt > 0)
            spozzAmount.add(avatarCnt.mul(airdropAmountPerAvatar));
        ...
        if(nfts.length > 0){
            for(uint256 i = 0; i < nfts.length; i ++){
                if (isNFTRegistered[nfts[i]] == true) {
                    ...
                    spozzAmount.add(cnt.mul(airdropAmountPerNFT));
                }
            }
        }
        ...
        if(xnfts.length > 0){
            for(uint256 i = 0; i < xnfts.length; i ++){
                if (isXNFTRegistered[xnfts[i]] == true) {
                    ...
                    spozzAmount.add(cnt.mul(airdropAmountPerSpecialNFT));
                }
            }
        }
